...
- Points are computed by the shared `computeMatchPoints(match, isWalkover)` helper (used by both `updateMatchWithSubmission` and its exact inverse `reverseMatchStatsInto`). A non-walkover RR group-stage win scores **3 points** (`isWalkover ? 1 : 3`) — raised from 2 as a deliberate points-balance change, not a bug; keep this value in sync between the two call sites since they must stay exact inverses of each other.
