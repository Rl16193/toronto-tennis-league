  ...
  return (
    <div className="max-w-xl mx-auto px-4 sm:px-6 lg:px-8 pb-20 pt-4 md:pt-6">
      <div className="flex items-center gap-2 mb-5">
        ...
        <h1 className="text-2xl md:text-3xl font-display font-bold text-fg">History</h1>
      </div>

      <p className="text-[11px] font-bold uppercase tracking-widest text-fg/40 mb-3">My Matches</p>
      ...
      {loading ? (
        ...
        <div className="rounded-3xl bg-tennis-surface/30 border border-fg/5 py-12 text-center">
          <p className="text-sm text-fg/40">No matches yet — join an event and your results will land here.</p>
        </div>
      ) : (
        <div className="rounded-3xl bg-tennis-surface/30 border border-fg/5 overflow-hidden divide-y divide-fg/5">
          {matches.map((m, i) => (
            <motion.div key={m.id} {...fadeUp} transition={{ ...fadeUp.transition, delay: staggerDelay(i) }} className="flex items-center gap-3 px-4 py-3.5">
              ...
              <span className="text-xs font-bold text-fg/60 tabular-nums shrink-0">{m.scoreLine || '—'}</span>
            </motion.div>
          ))}
        </div>
      )}

      <p className="text-[11px] font-bold uppercase tracking-widest text-fg/40 mb-3 mt-8">Past Tournaments</p>
      ...
      {eventsLoading ? (
        ...
        <div className="rounded-3xl bg-tennis-surface/30 border border-fg/5 py-10 text-center">
          <p className="text-sm text-fg/40">No completed tournaments yet.</p>
        </div>
      ) : (
        <div className="rounded-3xl bg-tennis-surface/30 border border-fg/5 overflow-hidden divide-y divide-fg/5">
          {pastEvents.map((e, i) => (
