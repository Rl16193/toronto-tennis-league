...
const getRoundTone = (round: string) => {
  ...
  return 'bg-tennis-dark/40 border-white/10';
};
...
const BracketPlayer: React.FC<{ name: string; winner: boolean }> = ({ name, winner }) => (
  <div className={`h-8 border-b border-white/10 flex items-center px-2 text-sm font-semibold ${winner ? 'text-clay' : 'text-white/90'}`}>
...
const PlayerSelect: React.FC<PlayerSelectProps> = ({ matchId, slot, currentUserId, currentName, players, onSelect }) => {
  ...
  return (
    <div className="h-8 border-b border-white/10 flex items-center px-1 bg-clay/10">
      <select
        value={selectValue}
        onChange={(e) => {
        ...
        className="w-full text-xs bg-transparent border-none outline-none cursor-pointer text-white [&>option]:text-black"
      >
...
export const BracketView: React.FC<Props> = ({
  ...
  return (
    <section className="overflow-x-auto rounded-[2rem] bg-tennis-surface/20 text-white border border-white/10 p-4 md:p-6">
      {editMode && (
          ...
          >
            <div className="sticky top-0 z-10 bg-inherit pb-2">
              {(() => {
                ...
                return (
                  <p className={`text-center text-xs uppercase tracking-widest font-black ${rs === 'finished' ? 'text-clay' : rs === 'loading' ? 'text-white/40' : 'text-white'}`}>
              ...
              {onUpdateDeadline ? (
                <input
                  ...
                  className="mt-0.5 w-full text-center text-[10px] text-white/70 bg-transparent border-none outline-none cursor-pointer hover:text-white focus:text-white [color-scheme:dark]"
            ...
            {round.matches.map((match, matchIndex) => {
              ...
              return (
                ...
                >
                  <div className="relative rounded-sm bg-tennis-dark/60 border border-white/10 shadow-sm">
                    {/* Status dot */}
                    {showDot && (
                      <span
                    ...
                    {/* Score display */}
                    {scoreText && (
                      <div className="border-t border-white/10 px-2 py-0.5 text-[10px] text-white/60 font-mono tracking-wide">
                        {scoreText}
                      </div>
                    )}
                    ...
                    {round.round === 'F' && match.winner_name ? (
                      <div className="border-t border-white/10 px-2 py-1 text-xs font-black text-clay">
                    ...
                    {showCreatorSubmit && (
                      <button
                        ...
                        className="w-full border-t border-white/10 px-2 py-1 text-[10px] text-white/70 hover:text-clay transition-colors text-center leading-tight"
                      >
                    ...
                    {showPlayerSubmit && (
                      alreadySubmitted ? (
                        <div className="w-full border-t border-white/10 px-2 py-1 text-[10px] text-green-400 text-center leading-tight">
                  ...
                  {roundIndex < rounds.length - 1 ? (
                    <div className="grid h-full grid-cols-[1fr_1px] items-center">
                      ...
                      <div className="h-1/2 border-r border-white/20" />
                    </div>
                  ) : null}
                </div>
              );
