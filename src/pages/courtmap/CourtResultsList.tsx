          ...
          {courts.map((c) => (
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-1">
                <p className="font-semibold text-white text-sm leading-snug">{c.dropdown || c.name}</p>
                {userCoords && <span className="text-clay font-medium text-xs shrink-0">{formatDist(c.distKm)}</span>}
              </div>
              {c.address && <p className="text-white/80 text-xs mb-1.5">{c.address}</p>}
              ...
              {c.clubInfo && <p className="text-white/80 text-xs mt-1 leading-snug">{c.clubInfo}</p>}
            </button>
          ))}
        </div>
      )}
