...
export function FilterSelect({
  ...
  return (
    <div className={`flex flex-col gap-0.5 ${disabled ? 'opacity-35 pointer-events-none' : ''}`}>
      ...
      >
        <option value="" style={selStyle}>
          All
        </option>
        {options.map((o) => (
          <option key={o.value} value={o.value} style={selStyle}>
            {o.label}
          </option>
        ))}
      </select>
  ...
  return (
    <>
      {entries.map((pb, idx) => {
        ...
        return (
          <Badge key={idx} bg="#431407" color="#fb923c">
            {label}
          </Badge>
        );
      })}
...
export const CourtPopup: React.FC<{
  ...
  <div style={{ fontFamily: 'system-ui, sans-serif', padding: '4px 2px', textAlign: 'center' }}>
    ...
    {court.address && <p style={{ color: '#6b7280', fontSize: 11, marginBottom: 7, marginTop: 0 }}>{court.address}</p>}
    ...
    {court.clubInfo && (
      <p style={{ color: '#6b7280', fontSize: 10, margin: '0 0 6px', lineHeight: 1.4 }}>{court.clubInfo}</p>
    )}
...
export const CourtResultsList: React.FC<CourtResultsProps> = React.memo(
  ({ courts, loading, userCoords, onSelectCourt, onViewPrograms }) => {
    if (loading) {
      return (
              ...
              >
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1">
                    ...
                    {c.lights && (
                      <Badge bg="#422006" color="#fbbf24">
                        LIGHTS
                      </Badge>
                    )}
                    {hasPublicHours(c) && (
                      <Badge bg="#1e3a5f" color="#93c5fd">
                        OPEN HOURS
                      </Badge>
                    )}
                    {c.bookingUrl && (
                      <Badge bg="#7c2d12" color="#fdba74">
                        BOOKABLE
                      </Badge>
                    )}
                    {c.count > 0 && (
                      <Badge bg="#14532d" color="#86efac">
