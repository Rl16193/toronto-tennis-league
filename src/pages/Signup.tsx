...
export const Signup: React.FC = () => {
  ...
  const { handleGoogleSignIn } = useGoogleSignIn({
    ...
    onAccountExists: (email, credential) => {
      ...
      setPhase('login');
      setStatusMessage(
        'This email already has a password. Enter it below to sign in — this will also enable Google sign-in for next time.',
      );
    },
  });
  ...
  const { handleAppleSignIn } = useAppleSignIn({
    ...
    onAccountExists: (email, credential) => {
      ...
      setPhase('login');
      setStatusMessage(
        'This email already has a password. Enter it below to sign in — this will also enable Apple sign-in for next time.',
      );
    },
  });
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="relative">
                          ...
                          <button
                            ...
                            className="absolute right-3 top-1/2 -translate-y-1/2 text-fg transition-colors"
                            tabIndex={-1}
                          >
            ...
            {phase === 'done' && (
              <div className="relative max-w-md mx-auto text-center rounded-3xl border border-green-500/25 bg-green-500/5 p-8">
                ...
                <p className="text-fg/70 text-sm leading-relaxed">
                  A welcome email containing further instructions has been sent. Please check your junk / spam folder if
                  you don't see it.
                </p>
                      ...
                      {(["Men's", "Women's"] as const).map((league) => (
                        <button
                          ...
                          className={`px-4 py-2 rounded-full text-xs font-bold transition-all ${
                            formData.league === league ? 'bg-clay text-white' : 'bg-fg/5 text-fg hover:bg-fg/10'
                          }`}
                        >
                          {league}
                        </button>
