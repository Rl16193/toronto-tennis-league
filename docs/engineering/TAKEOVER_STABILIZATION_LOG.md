...
| Current   | Emulator wiring had no reusable synthetic dataset or safe seed command.                                             | `tests/fixtures/local-fixtures.mjs`, `tests/fixtures/seed-emulator.mjs`, package scripts, README             | 31 Firestore documents and 4 Auth fixtures are defined; seed command refuses any project other than `rands-local` and any non-local Auth host. | Emulator suite must be running for seeding.                                                                                       |
