...
- **Tailwind's `dark:` variant is still used 0 times.** A raw grep for `dark:` returns 3, but all
  three are the variable *names* `--color-clay-dark:` and `--color-tennis-dark:` in `index.css`.
...
Their *labels, structures and behaviours* are very unlikely to have moved — the census shows the
