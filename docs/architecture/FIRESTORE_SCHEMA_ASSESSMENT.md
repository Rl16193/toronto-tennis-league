...
| Moderate | Public-read collections need an explicit field-sensitivity contract.                                                             | `users`, `stats`, `preferences`, `tasks`, `site_stats`, `providers`, `services`, and `listings` are readable broadly; public documents must not accumulate private fields.                                             |
