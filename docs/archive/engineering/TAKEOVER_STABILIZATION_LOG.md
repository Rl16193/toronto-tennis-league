...
| `5057d5e` | Architecture knowledge was distributed across source without a current-state record.                                | `docs/archive/architecture/` Markdown and ADRs                                                                     | Source, rules, Functions, Firebase config, and package manifests inspected.                                                                    | Deployed Firebase state and staging isolation remain unverified.                                                                  |
...
| `d636fb3` | Separate HTML/SVG diagram artifacts were difficult to view and maintain.                                            | `docs/archive/architecture/diagrams/*.md`, architecture links, skill inventory                                     | Six Mermaid Markdown files, one block each; no HTML/SVG references remain.                                                                     | Mermaid rendering depends on the Markdown viewer; no separate export is maintained.                                               |
...
| `4372baa` | Notification email delivery had no explicit local/staging boundary.                                                 | `functions/lib/emailDelivery.js`, `functions/lib/notify.js`, `docs/archive/runbooks/RESEND_DOMAIN_VERIFICATION.md` | 4 email-policy tests pass; emulators are blocked from sending and non-production requires an exact allowlist.                                  | Resend/DNS verification and authorized staging configuration remain external gates.                                               |
...
- Architecture diagrams are now six Mermaid Markdown files under `docs/archive/architecture/diagrams/`; the former HTML/SVG pairs and project-local diagram skill were removed.
