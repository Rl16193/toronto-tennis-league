...
test('requireTrimmedString trims surrounding whitespace and enforces presence', () => {
  ...
  assert.throws(() => requireTrimmedString('   ', 'Missing value.'), (error) => {
    assert.ok(error instanceof HttpsError);
...
test('requireTrimmedString enforces maxLength after trimming', () => {
  assert.equal(
    requireTrimmedString('  a@b.co  ', 'Missing email.', { maxLength: 320 }),
    'a@b.co',
  );

  assert.throws(() => requireTrimmedString(` ${'a'.repeat(321)} `, 'Missing email.', {
    maxLength: 320,
  }), (error) => {
    assert.ok(error instanceof HttpsError);
    assert.equal(error.code, 'invalid-argument');
